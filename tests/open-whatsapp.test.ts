...
test('Abre una pestaña antes de guardar y la dirige a WhatsApp después', async () => {
    ...
    expect(tab.location.replace).toHaveBeenCalledWith('https://wa.me/5491112345678?text=Hola%20mundo');
});

test('No guarda si el navegador bloquea la pestaña y la cierra si falla el guardado', async () => {
    const open = vi.spyOn(window, 'open').mockReturnValue(null);
    const save = vi.fn();
    await expect(openWhatsApp('12345678', 'Hola', save)).rejects.toThrow('ventanas emergentes');
    expect(save).not.toHaveBeenCalled();
    const tab = { opener: window, location: { replace: vi.fn() }, close: vi.fn() };
    open.mockReturnValue(tab as unknown as Window);
    await expect(openWhatsApp('12345678', 'Hola', async () => { throw new Error('Stock'); })).rejects.toThrow('Stock');
