...
vi.mock("@/components/providers/SiteSettingsProvider", () => ({
  useSiteSettings: () => ({ settings: { whatsapp: "5492235551234" } }),
}));
afterEach(cleanup);
...
test("WhatsApp conserva los datos enviados aunque el formulario se deshabilite mientras espera", async () => {
  ...
  for (const [label, value] of [
    ...
    ["Negocio o emprendimiento", "Mi negocio"],
    ["Teléfono con código de país", "+5492235551234"],
  ...
  fireEvent.submit(
    screen.getByRole("button", { name: "Enviar solicitud" }).closest("form")!,
  );
  expect(
    (
      screen
        .getByLabelText("Nombre y apellido")
        .closest("fieldset") as HTMLFieldSetElement
    ).disabled,
  ).toBe(true);
  expect(
    screen.queryByRole("link", { name: "Avisar al local por WhatsApp" }),
  ).toBeNull();
  await act(async () =>
    finish({
      ok: true,
      json: async () => ({ message: "Solicitud recibida." }),
    }),
  );
  const link = screen.getByRole("link", {
    name: "Avisar al local por WhatsApp",
  });
