...
const { save, remove, upload } = vi.hoisted(() => ({
  save: vi.fn(),
  remove: vi.fn(),
  upload: vi.fn(),
}));
vi.mock("@/services/ImageService", () => ({
  ImageService: { deleteImage: remove, uploadImage: upload },
}));
vi.mock("@/components/providers/SiteSettingsProvider", () => ({
  useSiteSettings: () => ({
    settings: {
      name: "El Arcángel",
      address: "Alberti 6062",
      hours: "Lunes a viernes de 8 a 18",
      whatsapp: "5492233422597",
      instagram: "https://www.instagram.com/elarcangelelarcangel/",
      homeContent: {
        ...defaultHomeContent,
        heroImageUrl: "https://res.cloudinary.com/demo/old.jpg",
        heroImagePublicId: "home/old",
      },
    },
    updateSettings: save,
  }),
}));
vi.mock("@/components/admin/draftImage/DraftImage", () => ({
  default: ({
    onChange,
  }: {
    onChange: (image: { url: string; public_id: string; file: File }) => void;
  }) => (
    <button
      onClick={() =>
        onChange({
...
test("La imagen y los textos se guardan juntos; la anterior solo se elimina después del éxito", async () => {
  ...
  expect(save).toHaveBeenCalledWith({
    homeContent: expect.objectContaining({
      heroTitle: "Nuevo título",
      heroImagePublicId: "home/new",
    }),
  });
  await user.click(screen.getByText("Guardar cambios"));
...
test("El footer muestra la configuración del local y sus enlaces reales", () => {
  ...
  expect(
    screen
      .getByRole("link", { name: "WhatsApp +5492233422597" })
      .getAttribute("href"),
  ).toBe("https://wa.me/5492233422597");
  expect(
    screen.getByRole("link", { name: "Instagram" }).getAttribute("href"),
  ).toBe("https://www.instagram.com/elarcangelelarcangel/");
});
