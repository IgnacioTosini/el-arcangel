...
test("Cancelar recibe el foco y cierra sin eliminar; el foco vuelve al botón", async () => {
  ...
  render(
    <DeleteRecordButton
      ...
      recordName="Buda decorativo"
      onDelete={remove}
    />,
  );
  const trigger = screen.getByRole("button", {
    name: "Eliminar producto: Buda decorativo",
  });
...
test("La confirmación evita duplicados y no se cierra mientras elimina", async () => {
  ...
  await user.click(
    screen.getByRole("button", { name: "Eliminar producto: Buda" }),
  );
...
test("Un error mantiene la confirmación abierta y permite reintentar", async () => {
  ...
  const remove = vi
    .fn()
    .mockResolvedValueOnce("No se pudo eliminar.")
    .mockResolvedValueOnce(null);
  render(
    <DeleteRecordButton
      label="Eliminar categoría"
      recordName="Regalería"
      onDelete={remove}
    />,
  );
  await user.click(
    screen.getByRole("button", { name: "Eliminar categoría: Regalería" }),
  );
  await user.click(screen.getByRole("button", { name: "Sí, eliminar" }));
