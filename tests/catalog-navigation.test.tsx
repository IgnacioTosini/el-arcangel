...
const { push, location } = vi.hoisted(() => ({ push: vi.fn(), location: { search: 'q=lavanda' } }));
vi.mock('next/navigation', () => ({ usePathname: () => '/catalogo', useSearchParams: () => new URLSearchParams(location.search), useRouter: () => ({ push }) }));
...
afterEach(() => { cleanup(); location.search = 'q=lavanda'; });
...
test('el catálogo pagina de a 12 conservando filtros y reinicia al cambiar categoría', () => {
    ...
    const history = vi.spyOn(window.history, 'pushState').mockImplementation(() => {});
    const products: CatalogProduct[] = Array.from({ length: 13 }, (_, index) => ({ id: String(index), name: `Lavanda ${index}`, sku: String(index), category: 'Aromas', categorySlug: 'aromas', createdAt: '2026-01-01', imageUrl: '/image-placeholder.svg', href: '/catalogo/lavanda', price: 10, availability: 'available' }));
    const props = { products, categories: [{ value: '', label: 'Todas' }, { value: 'aromas', label: 'Aromas' }] };
    const view = render(<Catalog {...props} />);
    expect(screen.getAllByRole('article')).toHaveLength(12);
    fireEvent.click(screen.getByRole('button', { name: 'Página siguiente' }));
    expect(history).toHaveBeenLastCalledWith(null, '', '/catalogo?q=lavanda&pagina=2');
