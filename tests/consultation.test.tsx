...
vi.mock("@/components/providers/SiteSettingsProvider", () => ({
  useSiteSettings: () => ({
    settings: { whatsapp: "5491112345678", instagram: "" },
  }),
}));
vi.mock("@/lib/open-whatsapp", () => ({
  openWhatsApp: vi.fn(async (_phone, _message, beforeOpen) => beforeOpen()),
}));
...
const product: CatalogProduct = {
  ...
  imageUrl: "/products/set-sahumerios.webp",
  href: "/catalogo/set-sahumerios",
  price: 6500,
...
test("El mínimo de unidades se exige aunque no haya importe mínimo", async () => {
  localStorage.setItem(
    ...
    JSON.stringify({
      items: [{ id: "variant-1", name: product.name, quantity: 1 }],
    }),
  );
  ...
  const quantity = screen.getByRole("spinbutton", {
    name: "Cantidad de Set sahumerios | Natural",
  });
...
test("Una lista recuperada con stock reducido bloquea el envío hasta corregir la cantidad", async () => {
  localStorage.setItem(
    "el-arcangel:consultation:v1",
    JSON.stringify({
      items: [{ id: "variant-1", name: product.name, quantity: 5 }],
    }),
  );
  const user = userEvent.setup();
  render(
    <ConsultationProvider>
      <Consultation
        products={[
          { ...product, variants: [{ ...product.variants![0], stock: 2 }] },
        ]}
      />
    </ConsultationProvider>,
  );
...
test("Una lista mixta conserva los artículos válidos y avisa que el subtotal excluye los faltantes", async () => {
  localStorage.setItem(
    ...
    JSON.stringify({
      items: [
        { id: "variant-1", name: product.name, quantity: 1 },
        { id: "deleted", name: "Artículo anterior", quantity: 4 },
      ],
    }),
  );
  const user = userEvent.setup();
  ...
  expect(
    await screen.findByRole("heading", { name: "Set sahumerios | Natural" }),
  ).toBeTruthy();
  expect(
    screen.getByRole("heading", { name: "Artículo anterior" }),
  ).toBeTruthy();
  ...
  expect(button.disabled).toBe(true);
  await user.click(
    screen.getByRole("button", { name: "Quitar Artículo anterior" }),
  );
  await user.type(screen.getByLabelText("Tu nombre"), "Cliente");
  await user.type(screen.getByLabelText("Teléfono de contacto"), "2235551234");
  expect(button.disabled).toBe(false);
  expect(
    screen.getByRole("heading", { name: "Set sahumerios | Natural" }),
  ).toBeTruthy();
});
...
test("Muestra los artículos guardados que ya no encuentra en el catálogo sin ocultar la lista", async () => {
  localStorage.setItem(
    "el-arcangel:consultation:v1",
    JSON.stringify({
      items: [{ id: "old-variant", name: "Producto guardado", quantity: 5 }],
    }),
  );
  const user = userEvent.setup();
  render(
  ...
  expect(
    await screen.findByRole("heading", { name: "Producto guardado" }),
  ).toBeTruthy();
  expect(screen.getByText(/Cantidad guardada: 5/)).toBeTruthy();
  expect(
    screen.queryByRole("heading", { name: "Tu consulta está vacía" }),
  ).toBeNull();
  expect(
    (
  ...
  await user.click(
    screen.getByRole("button", { name: "Quitar Producto guardado" }),
  );
  expect(
    screen.getByRole("heading", { name: "Tu consulta está vacía" }),
  ).toBeTruthy();
});
...
test("El stock limita las tarjetas, la ficha y la edición de la consulta", async () => {
  ...
  await user.click(
    screen.getByRole("button", {
      name: "Agregar Set sahumerios a mi consulta",
    }),
  );
  ...
  await user.click(
    screen.getByRole("button", { name: "Agregar a mi consulta" }),
  );
  const cartInput = screen.getByRole("spinbutton", {
    name: "Cantidad de Set sahumerios | Natural",
  }) as HTMLInputElement;
...
test("La tarjeta y la ficha suman la misma variante en una fila editable y eliminable", async () => {
  ...
  expect(
    screen.getAllByRole("heading", { name: "Set sahumerios | Natural" }),
  ).toHaveLength(1);
  const quantity = screen.getByRole("spinbutton", {
    name: "Cantidad de Set sahumerios | Natural",
  });
  ...
  await user.click(
    screen.getByRole("button", { name: "Quitar Set sahumerios | Natural" }),
  );
  expect(
    screen.getByRole("heading", { name: "Tu consulta está vacía" }),
  ).toBeTruthy();
...
test("Las variantes distintas del mismo producto conservan sus propias cantidades", async () => {
  ...
  await user.selectOptions(
    screen.getByRole("combobox", { name: "Variante" }),
    "variant-2",
  );
  await user.click(
    screen.getByRole("button", { name: "Agregar a mi consulta" }),
  ...
  expect(
    screen.getByRole("heading", { name: "Set sahumerios | Lavanda" }),
  ).toBeTruthy();
  await user.click(
    screen.getByRole("button", { name: "Quitar Set sahumerios | Natural" }),
  );
  expect(
    (
      screen.getByRole("spinbutton", {
        name: "Cantidad de Set sahumerios | Lavanda",
      }) as HTMLInputElement
    ).value,
  ).toBe("1");
});
...
test("Solo abre WhatsApp y vacía la lista después de guardar; un error permite reintentar", async () => {
  ...
  await user.type(screen.getByLabelText("Teléfono de contacto"), "2235551234");
  await user.click(
    screen.getByRole("button", { name: "Continuar por WhatsApp" }),
  );
  ...
  await waitFor(() =>
    expect(openWhatsApp).toHaveBeenCalledWith(
      "5491112345678",
      expect.stringContaining("Set sahumerios | Natural"),
      expect.any(Function),
    ),
  );
  expect(
  ...
  expect(JSON.parse(fetchMock.mock.calls[0][1].body).idempotencyKey).toBe(
    JSON.parse(fetchMock.mock.calls[1][1].body).idempotencyKey,
  );
  cleanup();
  setup();
  expect(
...
test("sin teléfono no abre WhatsApp ni envía solicitudes; el mayorista usa su teléfono guardado", async () => {
  ...
  localStorage.setItem(
    "el-arcangel:consultation:v1",
    JSON.stringify({
      items: [{ id: "variant-1", name: product.name, quantity: 2 }],
    }),
  );
  const view = render(
    <ConsultationProvider>
  ...
  expect(
    screen.getByText(/Completá un teléfono de contacto válido/),
  ).toBeTruthy();
...
test("detalle cambia el precio principal, descuento y SKU con la variante elegida", async () => {
  const discounted = {
    ...
    variants: [
      product.variants![0],
      { ...product.variants![1], price: 5000, compareAtPrice: 7000 },
    ],
  };
  render(
    <ConsultationProvider>
      <ProductDetail product={discounted} />
    </ConsultationProvider>,
  );
...
test("la tarjeta muestra el descuento y limita el stock de la misma variante que agrega", async () => {
  const discounted = {
    ...
    variants: [
      product.variants![0],
      { ...product.variants![1], stock: 1, price: 5000, compareAtPrice: 7000 },
    ],
  };
  render(
    <ConsultationProvider>
      <Featured products={[discounted]} />
      <Consultation products={[discounted]} />
    </ConsultationProvider>,
  );
  expect(screen.getByLabelText(/Precio anterior/).tagName).toBe("DEL");
  const button = screen.getByRole("button", {
