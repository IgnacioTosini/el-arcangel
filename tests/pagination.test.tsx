...
function List({ count = 23, filter = '', initialPage = 1 }: { count?: number; filter?: string; initialPage?: number }) {
    const pagination = usePagination(Array.from({ length: count }, (_, index) => index + 1), filter, 10, initialPage);
    return <><ul>{pagination.items.map(item => <li key={item}>{item}</li>)}</ul><Pagination {...pagination} targetId="unused" onChange={pagination.setPage} /></>;
}
afterEach(cleanup);

test('el catálogo puede mostrar una única página con navegación deshabilitada', () => {
    const change = vi.fn();
    const view = render(<Pagination showSinglePage page={1} total={7} pageSize={12} targetId="unused" onChange={change} />);
    ...
    view.rerender(<Pagination showSinglePage page={1} total={0} pageSize={12} targetId="unused" onChange={change} />);
...
test('permite impedir la navegación para conservar un borrador', () => {
    ...
    render(<Pagination page={1} total={20} pageSize={10} targetId="unused" onChange={change} />);
    fireEvent.click(screen.getByRole('button', { name: 'Página siguiente' }));
