...
test("Solicitud, login, aprobación administrativa, revocación y cierre de sesión", async () => {
  ...
  try {
    expect(
      (
        await POST(
          request("/api/wholesale/account", "POST", {
            ...registration,
    ...
    expect(
      (await POST(request("/api/wholesale/account", "POST", registration)))
        .status,
    ).toBe(201);
    const account = await prisma.wholesaleAccount.findUniqueOrThrow({
      where: { email },
    });
    ...
    expect(
      (await PATCH(request("/api/admin/wholesale", "PATCH", change))).status,
    ).toBe(401);
    auth.allowed = true;
    expect(
      (await PATCH(request("/api/admin/wholesale", "PATCH", change))).status,
    ...
    expect(
      (
        await PATCH(
          request("/api/admin/wholesale", "PATCH", {
            ...change,
            status: "REJECTED",
          }),
        )
    ...
    expect(
      (
        await DELETE(
          request("/api/wholesale/account", "DELETE", undefined, cookie),
        )
      ).status,
    ).toBe(200);
...
test("Eliminar requiere administrador y mismo origen, revoca sesiones y conserva consultas", async () => {
  ...
  try {
    ...
    expect(
      (
        await deleteAccount(
          request("/api/admin/wholesale", "DELETE", { id: account.id }),
        )
      ).status,
    ).toBe(401);
    auth.allowed = true;
    expect(
      (
        await deleteAccount(
          new NextRequest(origin + "/api/admin/wholesale", {
            method: "DELETE",
    ...
    expect(
      (
        await deleteAccount(
          request("/api/admin/wholesale", "DELETE", { id: "" }),
        )
      ).status,
    ).toBe(400);
    expect(
    ...
    expect(
      await prisma.wholesaleAccount.findUnique({ where: { id: account.id } }),
    ).toBeNull();
    expect(await readWholesaleSession(token)).toBeNull();
    expect(
      await prisma.wholesaleSession.count({ where: { accountId: account.id } }),
    ).toBe(0);
    expect(
