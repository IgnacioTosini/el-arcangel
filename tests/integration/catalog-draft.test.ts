...
async function save(body: unknown, status = 200) {
    const response = await fetch(`${origin}/api/admin/catalog`, { method: 'POST', headers: { Cookie: cookie, Origin: origin, 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
    const result = await response.json();
    assert.equal(response.status, status, JSON.stringify(result));
    return result as AdminData & { id: string };
}
async function snapshot(id: string) {
    const response = await fetch(`${origin}/api/admin/data`, { headers: { Cookie: cookie } });
    const data: AdminData = await response.json();
    const product = data.Product.find(item => item.id === id);
    assert.ok(product);
    return { model: 'Product', record: product, variants: data.ProductVariant.filter(item => item.productId === id), images: data.ProductImage.filter(item => item.productId === id) };
}
test('Guardado conjunto y rollback del borrador', async () => {
    const suffix = randomUUID();
    try {
        assert.equal((await fetch(`${origin}/api/admin/catalog`, { method: 'POST' })).status, 401);
        const category = await save({ model: 'Category', record: { id: `draft-${randomUUID()}`, name: `Categoría ${suffix}`, slug: '', active: true, sortOrder: 0 } });
        owned.categories.push(category.id);
        assert.equal((await db.category.findUniqueOrThrow({ where: { id: category.id } })).slug, `categoria-${suffix}`);
        const payload = { model: 'Product', record: { id: `draft-${randomUUID()}`, name: `Producto ${suffix}`, slug: '', description: 'Descripción', active: true, categoryIds: category.id }, variants: [{ id: `draft-${randomUUID()}`, name: 'Única', sku: `TEST-${suffix}`, price: 123, active: true, stock: 5 }], images: [{ id: `draft-${randomUUID()}`, url: '/image-placeholder.svg', publicId: `test-${suffix}`, sortOrder: 0 }] };
        await save({ ...payload, record: { ...payload.record, categoryIds: '' } }, 400);
        ...
        await save({ ...before, record: { ...before.record, categoryIds: '' } }, 400);
        const categoryRecord = await db.category.findUniqueOrThrow({ where: { id: category.id } });
        await save({ model: 'Category', record: { ...categoryRecord, active: false } }, 400);
        ...
        assert.equal(await db.product.count({ where: { slug: `otro-${suffix}` } }), 0);
        const automatic = await snapshot(created.id);
        automatic.variants.push({ id: `draft-${randomUUID()}`, name: 'Automatic SKU', sku: '', active: true });
        await save(automatic);
        const generated = await snapshot(created.id);
        const autoVariant = generated.variants.find(item => item.name === 'Automatic SKU')!;
        assert.match(String(autoVariant.sku), /^ARC-[0-9]{6,}$/);
        const stableSku = autoVariant.sku;
        autoVariant.name = 'Renamed';
        autoVariant.sku = '';
        await save(generated);
        assert.equal((await snapshot(created.id)).variants.find(item => item.id === autoVariant.id)?.sku, stableSku);
    } finally {
