...
test("Publica los textos juntos, conserva datos del local y rechaza una edición inválida", async () => {
  if (
    ...
    throw new Error("Requiere base local");
  const previous = await prisma.siteSettings.findUnique({
    where: { id: "store" },
  });
  const content = {
    ...defaultHomeContent,
    heroTitle: "Inicio de prueba",
    aboutDescription: "Descripción de prueba",
  };
  const request = (homeContent: unknown) =>
  ...
  try {
    ...
    const saved = await prisma.siteSettings.findUniqueOrThrow({
      where: { id: "store" },
    });
    expect(readHomeContent(saved.homeContent)).toEqual(content);
    if (previous) expect(saved.whatsapp).toBe(previous.whatsapp);
    expect((await PATCH(request({ ...content, heroTitle: "" }))).status).toBe(
      400,
    );
    expect(
      readHomeContent(
        (
          await prisma.siteSettings.findUniqueOrThrow({
            where: { id: "store" },
          })
        ).homeContent,
      ),
    ...
    if (previous)
      await prisma.siteSettings.update({
        where: { id: "store" },
        data: { homeContent: previous.homeContent! },
      });
