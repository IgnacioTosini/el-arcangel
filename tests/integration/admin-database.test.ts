...
async function request(
  ...
  const response = await fetch(`${origin}/api/admin/data`, {
    ...
    ...(method !== "GET" ? { body: JSON.stringify({ model, record }) } : {}),
  });
...
test("ABM, consultas e integridad de los datos", async () => {
  ...
  try {
    ...
    await request(
      "PATCH",
      "Product",
      { id: ids.product, categoryIds: "" },
      400,
    );
    await request(
      "PATCH",
      "Category",
      { id: ids.category, active: false },
      400,
    );
    await request("DELETE", "Category", { id: ids.category }, 400);
    ids.variant = (
      await request("POST", "ProductVariant", {
        productId: ids.product,
        name: "Variante temporal",
        sku: "",
        price: 1234.5,
        stock: 3,
        active: true,
    ...
    const sku = (
      await db.productVariant.findUniqueOrThrow({ where: { id: ids.variant } })
    ).sku;
    assert.match(sku, /^ARC-[0-9]{6,}$/);
    await request("PATCH", "ProductVariant", {
      id: ids.variant,
      sku: "",
      name: "Renamed",
    });
    ...
    ids.image = (
      await request("POST", "ProductImage", {
        productId: ids.product,
    ...
    const secondImage = await request("POST", "ProductImage", {
      productId: ids.product,
      url: "/image-placeholder.svg",
      publicId: `second-${suffix}`,
      sortOrder: 1,
    });
    await request("PATCH", "ProductImage", {
      id: secondImage.id,
      primary: true,
    });
    assert.equal(
      (
        await db.productImage.findUniqueOrThrow({
    ...
    assert.equal(
      (await db.productImage.findUniqueOrThrow({ where: { id: ids.image } }))
        .sortOrder,
      1,
    );
    const badOrigin = await fetch(`${origin}/api/admin/data`, {
      method: "PATCH",
      headers: {
        Cookie: cookie,
      ...
      body: JSON.stringify({
        model: "Product",
        record: { id: ids.product, name: "No guardar" },
      }),
    });
    ...
    assert.ok(
      (await db.siteSettings.findUniqueOrThrow({ where: { id: "store" } }))
        .name,
    );
    const invalidSettings = await fetch(`${origin}/api/settings`, {
      method: "PATCH",
      headers: {
        Cookie: cookie,
        Origin: origin,
      ...
      body: JSON.stringify({ whatsapp: "invalido" }),
    });
    assert.equal(invalidSettings.status, 400);
    await request(
      "PATCH",
      "ProductVariant",
      { id: ids.variant, price: -1 },
      400,
    );
    await request(
      "POST",
      "Category",
      { name: "Duplicada", slug: `test-${suffix}` },
      400,
    );
    await request("PATCH", "ProductVariant", {
      id: ids.variant,
    ...
    assert.equal(
      data.ProductVariant.find((v) => v.id === ids.variant)?.price,
      4321.25,
    );
    assert.equal(
      data.Product.find((p) => p.id === ids.product)?.categoryIds,
      ids.category,
    );
    assert.equal(
      data.ProductImage.find((i) => i.id === ids.image)?.productId,
      ids.product,
    );
    for (const path of [
      "/",
      "/catalogo",
      `/catalogo/test-${suffix}`,
      "/mi-consulta",
    ]) {
      ...
      if (path !== "/mi-consulta")
        assert.ok(html.includes("Producto temporal"), path);
    }
    ...
    const tooMany = await fetch(`${origin}/api/inquiries`, {
      method: "POST",
      headers: { Origin: origin, "Content-Type": "application/json" },
      body: JSON.stringify({
        ...inquiry,
        idempotencyKey: randomUUID(),
        items: [{ variantId: ids.variant, quantity: 4 }],
      }),
    });
    ...
    const savedOrder = await db.order.findUniqueOrThrow({
      where: { idempotencyKey: inquiryKey },
      include: { items: true },
    });
    ids.order = savedOrder.id;
    assert.equal(savedOrder.estimatedTotal?.toNumber(), 8642.5);
    ...
    await request(
      "PATCH",
      "Order",
      { id: ids.order, status: "COMPLETED" },
      400,
    );
    assert.equal(
      (await db.order.findUniqueOrThrow({ where: { id: ids.order } })).status,
      "PENDING",
    );
    assert.equal(
      (
    ...
    assert.ok(
      (await db.order.findUniqueOrThrow({ where: { id: ids.order } }))
        .completedAt,
    );
    assert.equal(
      (
    ...
    assert.equal(
      (await fetch(`${origin}/catalogo/test-${suffix}`)).status,
      404,
    );
