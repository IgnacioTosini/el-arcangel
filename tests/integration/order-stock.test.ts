...
async function fixture(stocks: (number | null)[], quantity = 1) {
  ...
  const product = await db.product.create({
    data: {
      name: "Prueba de stock",
      slug: `stock-${suffix}`,
      description: "Prueba temporal",
      active: false,
      variants: {
        create: stocks.map((stock, index) => ({
          name: `Variante ${index}`,
          sku: `STOCK-${suffix}-${index}`,
          stock,
        })),
      },
    },
...
test("Stock sin definir impide el cierre y conserva todas las existencias", async () => {
  ...
  expect(
    (await db.order.findUniqueOrThrow({ where: { id } })).completedAt,
  ).toBeNull();
});
...
test("Una segunda consulta no se completa cuando la primera agotó el stock", async () => {
  ...
  expect(
    (await db.order.findUniqueOrThrow({ where: { id: second } })).status,
  ).toBe("PENDING");
  expect(
    (
...
test("Dos confirmaciones concurrentes no pueden consumir las mismas existencias", async () => {
  ...
  expect(
    await db.order.count({
      where: { id: { in: [first, second] }, status: "COMPLETED" },
    }),
  ).toBe(1);
  expect(
    (
