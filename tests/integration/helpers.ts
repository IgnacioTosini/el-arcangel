...
export async function integrationContext() {
  ...
  if (
    !localHosts.includes(new URL(origin).hostname) ||
    !localHosts.includes(new URL(process.env.DATABASE_URL ?? "").hostname)
  ) {
  ...
  try {
    ...
    throw new Error(
      "Iniciá Next.js y PostgreSQL con la misma configuración antes de ejecutar las pruebas de integración.",
    );
  }
