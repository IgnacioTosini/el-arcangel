...
vi.mock('next/navigation', () => ({ useRouter: () => ({ push }), usePathname: () => '/catalogo', useSearchParams: () => new URLSearchParams('categoria=aromas') }));
afterEach(cleanup);
...
test('El modo en tiempo real actualiza la URL conservando filtros sin navegar ni pedir sugerencias', () => {
    ...
    expect(replace).toHaveBeenLastCalledWith(null, '', '/catalogo?categoria=aromas&q=lavanda');
...
test('Las sugerencias permiten abrir una ficha con el teclado', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ ok: true, json: async () => [{ id: '1', name: 'Quemador', href: '/catalogo/quemador', imageUrl: '/image-placeholder.svg' }] }));
