...
vi.mock("@/components/admin/productEditor/ProductEditor", () => ({
  default: ({ product }: { product: { name: string } }) => (
    <article>{product.name}</article>
  ),
}));
vi.mock("@/components/admin/categoryEditor/CategoryEditor", () => ({
  default: ({ category }: { category: { name: string } }) => (
    <article>{category.name}</article>
  ),
}));
vi.mock("@/components/admin/orderCard/OrderCard", () => ({
  default: () => null,
}));
vi.mock("@/components/admin/adminOverview/AdminOverview", () => ({
  statusLabels: {},
}));
afterEach(() => {
  cleanup();
