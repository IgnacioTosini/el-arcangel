...
vi.mock("axios", () => ({
  default: { post: vi.fn(), delete: vi.fn(), isAxiosError: vi.fn(() => false) },
}));
...
function canvasEnvironment({
  ...
  const canvas = {
    width: 0,
    height: 0,
    getContext: () => (context ? { drawImage: vi.fn() } : null),
    toBlob: (callback: (blob: Blob | null) => void) =>
      callback(
        nullBlob
          ? null
          : new Blob(["a".repeat(blobSize)], { type: "image/webp" }),
      ),
  };
  vi.stubGlobal(
    "Image",
...
test("Optimización proporcional y liberación del object URL", async () => {
  ...
  const result = await ImageService.optimizeImage(
    new File(["a".repeat(100)], "foto.jpg", { type: "image/jpeg" }),
  );
...
test.each([{ nullBlob: true }, { context: false }])(
  "Rechaza canvas inválido y libera el recurso: %j",
  async (options) => {
    const { revoke } = canvasEnvironment(options);
    await expect(
      ImageService.optimizeImage(
        new File(["a"], "foto.png", { type: "image/png" }),
      ),
    ).rejects.toThrow();
...
test("Valida respuestas del servidor", async () => {
  ...
  const good = {
    ...
    public_id: "demo/productos/a",
  };
  vi.mocked(axios.post)
    .mockResolvedValueOnce({ data: good })
    .mockResolvedValueOnce({ data: { success: true } });
  expect(await ImageService.uploadImage(file, { folder: "productos" })).toEqual(
    good,
  );
  expect(
    (await ImageService.uploadImage(file, { folder: "productos" })).success,
  ).toBe(false);
});
