...
export async function PATCH(req: NextRequest) {
    ...
    try {
        ...
        const units = body.wholesaleMinimumUnits === undefined ? undefined : Number(body.wholesaleMinimumUnits);
        if (units !== undefined && (!['string', 'number'].includes(typeof body.wholesaleMinimumUnits) || !Number.isSafeInteger(units) || units < 2 || units > 99900)) throw new Error();
        if ('wholesaleMinimum' in body) {
            const value = body.wholesaleMinimum;
            if (typeof value !== 'string' || !/^\d{1,10}(\.\d{1,2})?$/.test(value)) throw new Error();
        ...
        const update = { ...data, ...(units === undefined ? {} : { wholesaleMinimumUnits: units }), ...('homeContent' in body ? { homeContent: validateHomeContent(body.homeContent) } : {}) };
        return Response.json(await prisma.siteSettings.upsert({ where: { id: 'store' }, create: { id: 'store', ...update }, update }));
    }
    catch {
