...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const account = await readWholesaleSession(req.cookies.get(wholesaleCookie)?.value);
        ...
        const order = await prisma.$transaction(async (tx) => {
            if (purchaseType === 'WHOLESALE' && !await tx.wholesaleAccount.findFirst({ where: { id: account!.id, status: 'APPROVED' } })) throw new Error('Cuenta sin aprobación');
            ...
            if (variants.length !== items.length || variants.some(v => v.stock === 0))
                throw new Error();
            const settings = purchaseType === 'WHOLESALE' ? await tx.siteSettings.findUnique({ where: { id: 'store' } }) : null;
            if (purchaseType === 'WHOLESALE' && items.reduce((sum, item) => sum + item.quantity, 0) < (settings?.wholesaleMinimumUnits ?? 2)) throw new MinimumOrderError(`El pedido mayorista requiere al menos ${settings?.wholesaleMinimumUnits ?? 2} unidades en total.`);
            let total = new Prisma.Decimal(0);
            let incomplete = false;
            const lines = items.map(item => {
                const variant = variants.find(v => v.id === item.variantId)!;
                const price = purchaseType === 'WHOLESALE' ? variant.wholesalePrice : variant.price;
                ...
                return { quantity: item.quantity, variantId: variant.id, productName: variant.product.name, variantName: variant.name, sku: variant.sku, unitPrice: price };
            });
            if (settings && total.lessThan(settings.wholesaleMinimum)) throw new MinimumOrderError('El pedido no alcanza el importe m\u00ednimo mayorista de $ ' + settings.wholesaleMinimum.toFixed(2) + '. Los productos sin precio no suman para alcanzar el m\u00ednimo.');
            return tx.order.create({ data: { wholesaleAccountId: purchaseType === 'WHOLESALE' ? account!.id : null, idempotencyKey: body.idempotencyKey, customerName: customer.name.trim(), customerPhone: customer.phone.trim(), businessName: purchaseType === 'WHOLESALE' ? account!.business : null, customerNote: customer.comment.trim() || null, purchaseType, estimatedTotal: incomplete ? null : total, items: { create: lines } }, select: { number: true } });
        });
        return Response.json(order);
    }
    catch (error) {
        if (error instanceof MinimumOrderError) return Response.json({ error: error.message }, { status: 400 });
