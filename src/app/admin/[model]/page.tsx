...
export function generateStaticParams() {
  return adminSections.map((model) => ({ model: model.slug }));
}
...
export default async function AdminModelPage({
  ...
  if (
    ...
    notFound();
  return (
    <Suspense fallback={<p>Cargando registros…</p>}>
      <AdminManagement key={model.name} section={model.name} />
    </Suspense>
  );
}
...
export async function generateMetadata({
  ...
  return {
    title: `${section?.label ?? "Administración"} | El Arcángel`,
