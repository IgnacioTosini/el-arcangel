...
const findProduct = cache(async (slug: string) => {
  return (await readCatalog()).products.find(
    (product) => product.href === `/catalogo/${slug}`,
  );
});
...
export async function generateMetadata({
  ...
  const description =
    ...
    `Consultá por ${product.name} en El Arcángel. Venta por mayor y menor.`;
  return pageMetadata(
    `${product.name} | El Arcángel`,
    description,
    product.href,
    product.imageUrl,
  );
