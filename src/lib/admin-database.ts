...
export async function readAdminData(): Promise<AdminData> {
    const [Category, products, variants, ProductImage, orders, items] = await Promise.all([
    ...
    return JSON.parse(JSON.stringify({ Category, Product, ProductVariant, ProductImage, Order, OrderItem, Cart: [], CartItem: [] })) as AdminData;
}
...
export function writableFields(model: ModelName, record: AdminRecord) {
    ...
    for (const field of fields) {
        if (field.readonly || !(field.name in record))
            continue;
        ...
        if (['Int', 'Decimal'].includes(field.type)) {
            ...
            const number = value;
            if (!Number.isFinite(number) || number < 0 || field.type === 'Int' && !Number.isSafeInteger(number))
        ...
        if (field.type === 'String' && (typeof value !== 'string' || field.required && !value.trim()))
            throw new AdminValidationError(`Completá ${field.name}.`);
        if (field.name === 'slug' && (typeof value !== 'string' || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(value)))
    ...
    if (model === 'Product' && 'categoryIds' in record)
        result.categories = { set: String(record.categoryIds ?? '').split(',').filter(Boolean).map(id => ({ id })) };
    if (model === 'Order' && result.status)
        result.completedAt = result.status === 'COMPLETED' ? result.completedAt || new Date() : null;
    return result;
}
type Delegate = {
    create(args: {
...
export function adminDelegate(model: ModelName, db: Prisma.TransactionClient = prisma): Delegate {
    const delegates = { Category: db.category, Product: db.product, ProductVariant: db.productVariant, ProductImage: db.productImage, Order: db.order };
