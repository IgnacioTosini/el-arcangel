...
function signingKey() {
    ...
    return createHmac('sha256', process.env.ADMIN_SESSION_SECRET || password).update(`el-arcangel:admin:${password}`).digest();
}
...
export async function verifyAdminSessionToken(token?: string): Promise<boolean> {
    ...
    if (parts.length !== 2 || !/^\d+$/.test(parts[0]) || !/^[A-Za-z0-9_-]{43}$/.test(parts[1])) return false;
    const expiration = Number(parts[0]);
    if (!Number.isSafeInteger(expiration) || expiration <= Date.now() || expiration > Date.now() + ADMIN_SESSION_DURATION_SECONDS * 1000) return false;
