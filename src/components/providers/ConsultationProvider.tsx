...
type ConsultationContextValue = {
  ...
  clearItems: () => void;
  addItem: (
    item: Omit<ConsultationItem, "quantity">,
    quantity?: number,
  ) => void;
};
...
export default function ConsultationProvider({
  ...
  const storageKey = accountId
    ? `el-arcangel:consultation:wholesale:${accountId}`
    : "el-arcangel:consultation:v1";
  return (
    <ScopedConsultationProvider key={storageKey} storageKey={storageKey}>
      {children}
    </ScopedConsultationProvider>
  );
}

function ScopedConsultationProvider({
  children,
  storageKey,
}: {
  children: ReactNode;
      ...
      if (Array.isArray(saved?.items))
        for (const item of saved.items) {
          ...
          restored.set(item.id, {
            ...
            quantity: Math.min(
              999,
              (restored.get(item.id)?.quantity ?? 0) + item.quantity,
            ),
          });
        }
  ...
  useEffect(() => {
    ...
    try {
      ...
      /* Keep the consultation usable when browser storage is unavailable. */
    }
  ...
  function updateQuantity(id: string, quantity: number, stock?: number | null) {
    if (
      ...
      quantity < 1 ||
      quantity > Math.min(999, stock ?? 999)
    )
      return;
    setItems((current) =>
  ...
  function addItem(item: Omit<ConsultationItem, "quantity">, quantity = 1) {
    ...
    const added = Math.max(
      ...
      Math.min(
        quantity,
        limit - (items.find((entry) => entry.id === item.id)?.quantity ?? 0),
      ),
    );
    if (!added) {
      toast.info("Ya agregaste la cantidad disponible de esta variante.", {
        toastId: "consultation-limit",
      ...
      current.some((entry) => entry.id === item.id)
        ? current.map((entry) =>
            entry.id === item.id
              ? {
                  ...entry,
                  quantity: Math.min(limit, entry.quantity + quantity),
                }
              : entry,
          )
...
export function useConsultation() {
  ...
  if (!context)
    throw new Error(
      "useConsultation debe usarse dentro de ConsultationProvider",
    );
  return context;
}
