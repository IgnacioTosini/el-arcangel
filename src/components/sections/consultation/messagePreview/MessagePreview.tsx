...
export default function MessagePreview({ message, onSubmit }: { message: string; onSubmit: () => Promise<string> }) {
    ...
    async function sendInquiry() {
        if (busy.current || !whatsappReady) return;
        busy.current = true;
        setSaving(true);
        try {
            await onSubmit();
    ...
    return (
        <section className="messagePreviewContent" aria-label="Mensaje a enviar">
            ...
            <p className="messagePreviewNote">Completá tu nombre y teléfono. Al continuar, guardamos tu consulta para que el local pueda responderte y vaciamos tu lista. WhatsApp se abrirá con el mensaje preparado: presioná Enviar allí para terminar.</p>
            <div className="messagePreviewButtons"><button type="button" className="consultationButton consultationButtonPrimary" disabled={saving || !whatsappReady} onClick={sendInquiry}>{saving ? 'Preparando consulta…' : 'Continuar por WhatsApp'}</button>
