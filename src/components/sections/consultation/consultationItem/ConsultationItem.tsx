    ...
    return (
        <article ref={animationRef} className="consultationItemContent">
            ...
            <div className="consultationItemInfo">
                ...
                <p>{product.stock == null ? 'Stock a confirmar' : `${product.stock} disponibles`}</p>
                {quantity > limit && <p role="alert">El stock cambió. Reducí la cantidad o quitá este artículo.</p>}
                <span>{product.price === null ? 'Consultar precio' : `${product.priceFrom ? 'Desde ' : ''}${formatAmount(product.price)}`}</span>
            </div>
            <div className="consultationItemActions">
                <label htmlFor={id} className="consultationItemLabel">Cantidad de {product.name}</label>
                <input id={id} type="number" min={1} max={limit} step={1} required value={draft ?? quantity} onChange={(event) => {
                    const value = event.target.value;
                    setDraft(value);
                    const amount = Number(value);
                    if (Number.isInteger(amount) && amount >= 1 && amount <= limit) onQuantityChange(amount);
                }} onBlur={() => setDraft(null)} />
                <button type="button" onClick={onRemove} aria-label={`Quitar ${product.name}`}>Quitar</button>
            </div>
        </article>
    );
}
