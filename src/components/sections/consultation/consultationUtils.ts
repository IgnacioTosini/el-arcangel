...
export function buildMessage(
  ...
  return [
    ...
    type === "WHOLESALE" && customer.business.trim()
      ? `Negocio: ${customer.business.trim()}`
      : "",
    "",
    ...lines.map(({ product, quantity }) => {
      const priceText =
        product.price === null
          ? "Precio a confirmar"
          : `${product.priceFrom ? "Desde " : ""}${formatAmount(product.price)} por unidad`;

      return `• ${quantity} x ${product.name} (${product.sku}) — ${priceText}`;
    }),
    "",
    ...
    .filter(
      (line, index, all) => line !== "" || (index > 0 && all[index - 1] !== ""),
    )
    .join("\n");
}
