  ...
  return (
    <>
      <div className="productGalleryControls">
        <button
          type="button"
          onClick={() => setZoom((current) => clamp(current - 0.5, 1, 4))}
          disabled={zoom === 1}
      ...
      <div
        ref={viewportRef}
        className="productGalleryViewport"
        tabIndex={0}
        role="region"
        aria-label="Imagen ampliable. Usá más y menos para zoom, flechas para explorar y cero para restablecer."
        onPointerMove={(event) => {
          ...
          if (event.pointerType === "mouse") {
            ...
            const deltaX =
              ((event.clientX - touchRef.current.x) / bounds.width) * 100;
            const deltaY =
              ((event.clientY - touchRef.current.y) / bounds.height) * 100;
            setPosition((current) => ({
        ...
        onKeyDown={(event) => {
          ...
          if (event.key === "0") reset();
            ...
            setZoom((current) => clamp(current + 0.5, 1, 4));
          else if (event.key === "-")
            setZoom((current) => clamp(current - 0.5, 1, 4));
          else
            setPosition((current) => ({
      ...
      >
        <Image
          ...
          height={800}
          draggable={false}
          className="productGalleryExpanded"
          sizes="(max-width: 700px) 150vw, 1200px"
          style={{
