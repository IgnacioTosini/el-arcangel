...
export default function ProductCard({
  ...
  const variant = source.variants?.find(
    (item) => item.id === source.defaultVariantId,
  );
  const product: ProductCardData = variant
    ? {
        ...source,
        price: variant.price,
  ...
  return (
    <article ref={animationRef} className="productCardContent">
      ...
      <div className="productCardInner">
        ...
        <span
          className={`productCardAvailability productCardAvailability${product.availability}`}
        >
          {availabilityLabels[product.availability]}
        </span>
        <p className="productCardCategory">
          {product.defaultVariantName ? `${product.defaultVariantName}: ` : ""}
          {product.stock == null
            ? "Stock a confirmar"
        ...
        <div className="productCardButtons">
          <Link href={product.href} className="productCardButton">
            Ver detalle
          </Link>
          <button
            type="button"
