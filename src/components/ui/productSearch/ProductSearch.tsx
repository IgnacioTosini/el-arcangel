  ...
  return (
    <form
      ...
      onSubmit={(event) => {
        ...
        navigate(visible && selected >= 0 ? items[selected]?.href : undefined);
      }}
      onBlur={(event) => {
      ...
      <div className="productSearchField">
        <input
          ...
          aria-activedescendant={
            visible && selected >= 0 ? `${id}-${selected}` : undefined
          }
        ...
        />
        <button type="submit" aria-label="Ver resultados de búsqueda">
          Buscar
        </button>
      </div>
      {visible && (
        <div className="productSearchDropdown">
          <ul id={id} role="listbox" aria-label="Productos sugeridos">
            {items.map((item, index) => (
              ...
              >
                <Image src={item.imageUrl} alt="" width={40} height={48} />
                <span>{item.name}</span>
              </li>
            ))}
          </ul>
          {!ready ? (
            ...
            !items.length && (
              <p role="status">
                {result.error
                  ? "No pudimos cargar sugerencias. Probá ver los resultados."
