...
export default function CategorySelect({
  ...
  const matches = categories.filter((category) =>
    slugify(String(category.name)).includes(slugify(query)),
  );
  return (
    <div className="categorySelect">
      ...
      <button
        type="button"
        className="adminButton categorySelectTrigger"
      ...
      >
        <span id={`${id}-value`}>
          {selected.length
            ? `${selected.length} seleccionada${selected.length === 1 ? "" : "s"} · ${selected
                .slice(0, 2)
      ...
      {open && (
        ...
        >
          ...
          />
          <div className="categorySelectOptions">
            ...
            {!matches.length && <p>No hay categorías que coincidan.</p>}
          </div>
          <button
            type="button"
