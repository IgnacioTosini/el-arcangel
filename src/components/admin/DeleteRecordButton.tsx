...
export default function DeleteRecordButton({
  label,
  recordName,
  description = "El registro se quitará del sitio.",
  onDelete,
}: {
  label: string;
  recordName?: string;
  ...
  return (
    <>
      <button
        type="button"
        className="deleteRecordTrigger"
        aria-label={recordName ? `${label}: ${recordName}` : label}
      ...
      {confirm && (
        ...
        >
          <div className="deleteRecordConfirmation" aria-busy={busy}>
            ...
            {error && (
              <p className="deleteRecordError" role="alert">
                {error}
              </p>
            )}
