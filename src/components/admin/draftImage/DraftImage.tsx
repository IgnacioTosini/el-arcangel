...
export default function DraftImage({
  ...
  onChange: (image: DraftImageValue | null) => void;
  ...
  return (
    <div className="imageUploadContent">
      {url && (
        <Image src={url} alt="Imagen del borrador" width={120} height={120} />
      )}
      <div>
        <label htmlFor={id}>{url ? "Reemplazar imagen" : "Subir imagen"}</label>
        <input
          id={id}
          type="file"
        ...
        />
        <small>
          Vista previa local. La imagen se sube y publica al guardar los
          cambios.
        </small>
        {url && (
          <button
