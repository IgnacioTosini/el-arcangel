...
export default function WholesaleAccounts({ accounts }: { accounts: Account[] }) {
    ...
    async function remove(id: string): Promise<string | null> {
        ...
        setBusy(true);
        try {
            const response = await fetch('/api/admin/wholesale', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) });
            const result = await response.json();
            if (!response.ok) throw new Error(result.error);
            if (notification?.id === id) setNotification(null);
            if (change?.account.id === id) setChange(null);
            toast.success('Cuenta eliminada. Las consultas anteriores se conservaron.');
            router.refresh();
            return null;
        } catch (error) { return error instanceof Error ? error.message : 'No se pudo eliminar la cuenta.'; }
        finally { setBusy(false); }
    }
    function notificationLink(account: Account) {
        const message = account.status === 'APPROVED' ? 'Tu cuenta mayorista fue aprobada. Ya podés ingresar para ver precios y armar tu consulta.' : account.status === 'REJECTED' ? 'Tu cuenta no tiene acceso mayorista. Contactanos por este medio para revisar tu solicitud.' : 'Tu solicitud de cuenta mayorista está pendiente de revisión.';
        return whatsappLink(account.phone, `Hola ${account.name}, te escribimos de El Arcángel. ${message}`);
    }
    ...
    return <section className="wholesaleAccounts">
        ...
        <p className="adminMuted">Deshabilitar conserva la cuenta para volver a aprobarla. Eliminar borra la cuenta y sus sesiones, pero conserva las consultas anteriores; ese email podrá registrarse nuevamente.</p>
        {notification && <div className="wholesaleConfirmation" role="status"><p>Estado de {notification.name} guardado. Podés avisarle por WhatsApp; el mensaje no se envía automáticamente.</p>{notificationLink(notification) && <a className="adminButton adminButtonPrimary" href={notificationLink(notification)!} target="_blank" rel="noopener noreferrer">Avisar a {notification.name} por WhatsApp</a>}<button type="button" className="adminButton" onClick={() => setNotification(null)}>Cerrar aviso</button></div>}
        ...
        {shown.map(account => <article key={account.id}>
            ...
            {notificationLink(account) && <a className="adminButton" href={notificationLink(account)!} target="_blank" rel="noopener noreferrer">Avisar estado por WhatsApp</a>}
            <DeleteRecordButton label="Eliminar cuenta mayorista" recordName={`${account.name} (${account.email})`} description="Se eliminarán la cuenta y sus sesiones de acceso. Las consultas anteriores se conservarán. El email podrá volver a registrarse. Si solo querés quitar el acceso, cambiá el estado a Inactiva." onDelete={() => remove(account.id)} />
