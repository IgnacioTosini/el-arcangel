...
type InlineFieldsProps = {
  record: AdminRecord;
  fields: InlineField[];
  onSave: (record: AdminRecord) => string | null | Promise<string | null>;
  saveTogether?: boolean;
  children?: ReactNode;
  extraDirty?: boolean;
  disabled?: boolean;
  onCancel?: () => void;
...
export default function InlineFields({
  ...
  const dirty =
    ...
    fields.some(
      (field) =>
        field.name in pending &&
        pending[field.name] !== String(record[field.name] ?? ""),
    );
  async function saveAll() {
    ...
    try {
      ...
      setError("No se pudo guardar. Tus cambios se conservan para reintentar.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function commit(field: InlineField, raw: string) {
    ...
    if (!result)
      setPending((current) => {
        const next = { ...current };
        if (next[field.name] === raw) delete next[field.name];
        return next;
      });
  }
  return (
    <div className="inlineFieldsContent">
        ...
        >
          ...
          {field.type === "checkbox" ? (
            <input
              ...
              onChange={(event) =>
                void saveValue(field.name, event.target.checked)
              }
            />
          ) : field.type === "textarea" ? (
            <textarea
              ...
              id={`${id}-${field.name}`}
              rows={3}
              value={pending[field.name] ?? String(record[field.name] ?? "")}
              onChange={(event) =>
                setPending((current) => ({
                  ...current,
      ...
      {error && (
        <p className="adminError inlineFieldFull" role="alert">
          {error}
        </p>
      )}
      {saveTogether && (
        <div className="inlineFieldFull inlineFieldsActions">
          <p role="status">
            {saving
              ? "Guardando cambios…"
              : dirty
